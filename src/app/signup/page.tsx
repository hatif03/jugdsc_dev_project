...
import { signUp, signInWithGoogle } from '@/firebase/auth/auth';
...
const Login = () => {
  ...
  return (
    ...
    <div className="hero-content flex-col justify-center">
        ...
        <p className="py-6 text-slate-200 font-bold">Hurry Up! Join the largest community of like-minded developers.</p>
        </div>
        <button className="btn bg-red-500 text-white"  onClick={signInWithGoogle}>
        <Image src="/google.png" alt="me" width="16" height="16"/> SignUp with Google
        </button>
        <form action="" onSubmit={handleSubmit}>
            <div className="card flex-shrink-0 w-[full] max-w-sm shadow-2xl bg-white justify-center">
                <div className="card-body p-[20px] sm:p-[36px]">
                    ...
                    <button className="btn btn-primary bg-red-500 hover:bg-red-600" type='submit'>Sign Up</button>
                    <Link href="/login"><p className=' text-red-500 text-xs font-bold flex-wrap mt-2'>Already have an account?</p></Link>
                    </div>
                </div>
            </div>
        </form>
